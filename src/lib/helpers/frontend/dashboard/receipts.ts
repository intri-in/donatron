...
export async function getAllUserReceipts(url:string): Promise<Receipt[] | null>{
    // console.log("url", url)
...
export async function getReceiptDetails(id:string): Promise<Receipt | null>{

        return new Promise( (resolve, reject) => {
        const url_api=getAPIURL()+`receipts/detail/?id=${id}`
          ...
          .then((body: APIResponseData) =>{
            ...
            console.error("getReceiptDetails", e)
            return reject(e.message)
          })
      })
